...
| DCP PLUTO | `64uk-42ks` | `BldgArea` (gross-building-area source), `LotArea` (**now loaded** as `pluto_lotarea`; powers the industrial land-value coverage disclosure, BldgArea÷LotArea), `BldgClass`, `latitude`/`longitude` (distance-based comp ranking), **`NumFloors` (display-only Stories, 99.83% fill on office), `Address` (display-address fallback)**, `YearBuilt` | Bulk CSV / SODA | **Release `26v1`** (version string from the data; SODA `rowsUpdatedAt` 2026-05-28). Data dictionary 26v1 (May 2026); readme 24v4 (Nov 2024) | **2026-07-01 (re-pulled to add LotArea; SAME `26v1`/`rowsUpdatedAt` anchor, so BldgArea reproduced byte-identical — office/retail unchanged; 858,602 lots)** |
...
| DCP PLUTO — retail floor-area mix | `64uk-42ks` | `retailarea/officearea/resarea/bldgarea` for K (retail) lots, used to classify each retail parcel by MEASURED floor-area share into pure_retail / retail_office / retail_residential / retail_other (`retail_class` table; Stage 1 classifier + Stage 2 comp pools). PLUTO-derived; cites the PLUTO version. Retail screening is behind a test route only — public screen still refuses K (out_of_scope_v1) until Stage 3. | SODA API | PLUTO release (rowsUpdatedAt) | 2026-06-30 |
| DOF Property Abatement Detail | `rgyu-ii48` | Abatement disclosure: **ICAP** (comp tag + subject note) plus **J-51 / MCI / GCCA** (subject benefit note only). Key `parid` is space-padded to 30 chars (trim → 10-digit BBL); `tccode` space-padded. Filter to current snapshot = `max(extractdt)`, `tccode IN (ICAP, J51, MCI, GCCA)`, reduce to DISTINCT (BBL, program) in table `abatements_icap` (historical name kept for deployed-DB compatibility). Disclosure only — never changes the statutory tax. PILOT is NOT in this dataset. | SODA API | current snapshot `extractdt=2026-06-07` | 2026-07-16 |
| Roll exemption slice (same dataset) | `8y4t-faws` | Exemption disclosure: `curtxbextot` (exempt portion of taxable) per class-4 BBL → table `exemptions_class4` (`exempt_share = curtxbextot / curtxbtot`, final-period dedup shared with the loader). Feeds the per-comp "exempt N%" tag and the subject benefit note. Disclosure only — the plotted tax stays the full statutory amount. Re-run `python -m screener.exemptions` to bump. | SODA API | FY2027 final roll | 2026-07-16 |
